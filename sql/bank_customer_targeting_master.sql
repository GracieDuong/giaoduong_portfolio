/* =====================================================================
   ...
   T-SQL / SQL Server (SSMS / Azure Data Studio)

   PROJECT
   Data-Driven Customer Targeting for Banking Marketing Campaigns

   BUSINESS QUESTION
   How can the bank use customer data to identify customers with higher
   conversion propensity and improve campaign efficiency?

   ---------------------------------------------------------------------
   SOURCE
   Synthetic_Digital_Marketing_MIS DEMO TEST.xlsx
   Sheet: Customer_Data
   Import into: dbo.customer_campaign

   ACTUAL REFERENCE VALUES FROM THE PROVIDED DATASET
   --------------------------------------------------
   Total Customers             = 8,000
   Exposed Customers           = 5,432
   Clicked Customers           =   810
   Converted Customers         =   105
   Exposure Rate               = 67.90%
   CTR                         = 14.91%
   Conversion Rate (Exposed)   =  1.93%
   Click-to-Conversion Rate    = 12.96%
   Campaign Cost               = 3,642.65 M VND
   Revenue                     = 8,980.00 M VND
   CAC                         = 34.69 M VND / conversion
   Overall ROI                 = 146.52%

   IMPORTANT METHODOLOGY
   ---------------------
   1) Customer population analysis uses all 8,000 customers.

   2) Conversion analysis uses ONLY customers where:
          Campaign_Exposed_Flag = 1
      because conversion is observable only after campaign exposure.

   3) Baseline propensity target:
          Converted_Flag

   4) DO NOT use these as baseline propensity predictors:
          Clicked_Flag
          Campaign_Cost_Million_VND
          Revenue_Million_VND
          ROI
      They are post-campaign / outcome information and create leakage.

   5) Campaign_Channel and Product_Offered are analysed for business
      effectiveness, but are excluded from the baseline "WHO to target"
      propensity feature view. They can be introduced later in a separate
      Customer x Channel x Product / Next-Best-Action model.

   6) Gender is retained in the source for descriptive/fairness monitoring,
      but is excluded from the baseline propensity feature view below.

   7) SQL prepares data, calculates business KPIs, ranks model scores and
      validates targeting performance. Python / ML is responsible for
      generating validated out-of-fold (OOF) propensity probabilities.

   ===================================================================== */


/* #####################################################################
   SECTION 0 — SOURCE TABLE
   Run once if dbo.customer_campaign does not yet exist.
   Import the Excel Customer_Data sheet into this table afterward.
   ##################################################################### */

IF OBJECT_ID('dbo.customer_campaign', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.customer_campaign (
        Customer_ID                         VARCHAR(20)    NOT NULL,
        Age                                 INT            NULL,
        Gender                              VARCHAR(20)    NULL,
        Monthly_Income_Million_VND          DECIMAL(12,2)  NULL,
        Customer_Tenure_Months              INT            NULL,
        Account_Balance_Million_VND         DECIMAL(14,2)  NULL,
        App_Logins_Last_30D                 INT            NULL,
        Transactions_Last_30D               INT            NULL,
        Digital_Active_Flag                 TINYINT        NULL,
        Customer_Segment                    VARCHAR(50)    NULL,
        Campaign_Channel                    VARCHAR(50)    NULL,
        Product_Offered                     VARCHAR(50)    NULL,
        Campaign_Exposed_Flag               TINYINT        NULL,
        Clicked_Flag                        TINYINT        NULL,
        Converted_Flag                      TINYINT        NULL,
        Campaign_Cost_Million_VND           DECIMAL(12,2)  NULL,
        Revenue_Million_VND                 DECIMAL(14,2)  NULL,
        ROI                                 DECIMAL(18,6)  NULL,

        CONSTRAINT PK_customer_campaign
            PRIMARY KEY (Customer_ID),

        CONSTRAINT CK_customer_campaign_digital
            CHECK (Digital_Active_Flag IN (0,1)),

        CONSTRAINT CK_customer_campaign_exposed
            CHECK (Campaign_Exposed_Flag IN (0,1)),

        CONSTRAINT CK_customer_campaign_clicked
            CHECK (Clicked_Flag IN (0,1)),

        CONSTRAINT CK_customer_campaign_converted
            CHECK (Converted_Flag IN (0,1)),

        CONSTRAINT CK_customer_campaign_age
            CHECK (Age IS NULL OR Age >= 0),

        CONSTRAINT CK_customer_campaign_income
            CHECK (Monthly_Income_Million_VND IS NULL OR Monthly_Income_Million_VND >= 0),

        CONSTRAINT CK_customer_campaign_tenure
            CHECK (Customer_Tenure_Months IS NULL OR Customer_Tenure_Months >= 0),

        CONSTRAINT CK_customer_campaign_balance
            CHECK (Account_Balance_Million_VND IS NULL OR Account_Balance_Million_VND >= 0),

        CONSTRAINT CK_customer_campaign_app_logins
            CHECK (App_Logins_Last_30D IS NULL OR App_Logins_Last_30D >= 0),

        CONSTRAINT CK_customer_campaign_transactions
            CHECK (Transactions_Last_30D IS NULL OR Transactions_Last_30D >= 0),

        CONSTRAINT CK_customer_campaign_cost
            CHECK (Campaign_Cost_Million_VND IS NULL OR Campaign_Cost_Million_VND >= 0),

        CONSTRAINT CK_customer_campaign_revenue
            CHECK (Revenue_Million_VND IS NULL OR Revenue_Million_VND >= 0)
    );
END;
GO


/* Optional indexes for dashboard filtering / grouping. */
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IX_customer_campaign_exposed'
      AND object_id = OBJECT_ID('dbo.customer_campaign')
)
BEGIN
    CREATE INDEX IX_customer_campaign_exposed
        ON dbo.customer_campaign (Campaign_Exposed_Flag);
END;
GO
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IX_customer_campaign_segment'
      AND object_id = OBJECT_ID('dbo.customer_campaign')
)
BEGIN
    CREATE INDEX IX_customer_campaign_segment
        ON dbo.customer_campaign (Customer_Segment);
END;
GO
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IX_customer_campaign_channel'
      AND object_id = OBJECT_ID('dbo.customer_campaign')
)
BEGIN
    CREATE INDEX IX_customer_campaign_channel
        ON dbo.customer_campaign (Campaign_Channel);
END;
GO
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'IX_customer_campaign_product'
      AND object_id = OBJECT_ID('dbo.customer_campaign')
)
BEGIN
    CREATE INDEX IX_customer_campaign_product
        ON dbo.customer_campaign (Product_Offered);
END;
GO
